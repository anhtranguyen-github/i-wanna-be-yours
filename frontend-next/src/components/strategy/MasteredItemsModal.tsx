...
export function MasteredItemsModal({
    ...
    const getIcon = (type: string) => {
        switch (type) {
            ...
            default: return <BookOpen size={16} className="text-neutral-ink" />;
        }
    };
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4">
                        <span className="text-[10px] font-black text-neutral-ink uppercase tracking-widest block mb-1">Velocity</span>
                        <div className="flex items-baseline gap-1">
                            <span className="text-xl font-black text-neutral-ink">{velocity || 0}</span>
                            <span className="text-xs font-bold text-neutral-ink">{unit}/day</span>
                        </div>
                    </div>
                    <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4">
                        <span className="text-[10px] font-black text-neutral-ink uppercase tracking-widest block mb-1">Confidence</span>
                        <div className="flex items-baseline gap-1">
                            <span className="text-xl font-black text-neutral-ink">{confidence || 0}</span>
                            <span className="text-xs font-bold text-neutral-ink">%</span>
                        </div>
                    </div>
                    <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 col-span-2">
                        <span className="text-[10px] font-black text-neutral-ink uppercase tracking-widest block mb-1">Contributing Sources</span>
                        <div className="flex flex-wrap gap-2">
                            {contributing_task_types.length > 0 ? (
                                contributing_task_types.map(type => (
                                    <span key={type} className="px-2 py-0.5 bg-white border border-slate-200 rounded text-[10px] uppercase font-black text-neutral-ink">
                                        {type}
                                    </span>
                                ))
                            ) : (
                                <span className="text-xs font-bold text-neutral-ink">Standard Study Routines</span>
                            )}
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-brand-dark rounded-2xl p-5 text-white relative overflow-hidden  shadow-brand-dark/20">
                        ...
                        <div className="flex items-start gap-5 relative z-10">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-3 mt-2 pt-2 border-t border-white/10 font-black text-[10px] uppercase tracking-tighter text-neutral-ink">
                    ...
                    <div className="bg-slate-900 rounded-2xl p-5 text-slate-300 relative overflow-hidden border border-slate-800">
                        <div className="flex items-start gap-5 relative z-10">
                            <div className="w-12 h-12 rounded-xl bg-slate-800 flex items-center justify-center shrink-0">
                                <Brain className="text-neutral-ink" size={24} />
                            </div>
                            <div className="space-y-1">
                                <h3 className="font-black text-neutral-ink text-[10px] uppercase tracking-widest">Sensei Insight: Precision Mastery</h3>
                ...
                {filteredItems.length > 0 ? (
                            ...
                            <div key={item.id} className="group flex flex-col p-4 bg-white rounded-2xl border border-slate-100  hover: hover:border-brand-salmon/20 transition-all">
                                <div className="flex items-start justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="font-bold text-neutral-ink">{item.title}</h4>
                                            <div className="flex items-center gap-2 mt-0.5">
                                                <span className={cn(
                                                    "px-1.5 py-0.5 rounded text-[10px] font-bold uppercase",
                                                    item.level === 'N1' ? "bg-red-100 text-red-700" :
                                                        "text-[10px] font-bold uppercase tracking-tight",
                                                    item.status === 'burned' ? "text-neutral-ink" :
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="flex flex-col items-end">
                                            <span className="text-[10px] font-semibold text-neutral-ink uppercase tracking-tight">Proficiency</span>
                                            <span className={cn(
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-[10px] font-semibold text-neutral-ink uppercase tracking-tight">Last Activity</span>
                                            <span className={cn(
                    ...
                    <div className="text-center py-20 space-y-6 bg-slate-50/50 rounded-3xl border-2 border-dashed border-slate-200">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-neutral-ink max-w-sm mx-auto">
                                {safeItems.length === 0
                                    ? (current > 0
