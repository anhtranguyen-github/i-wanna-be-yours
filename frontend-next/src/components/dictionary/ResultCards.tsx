        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6  hover: transition-all">
            <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-2">
                    {data.audio || true && (
                        <button className="p-2.5 bg-slate-50 text-neutral-ink hover:text-brand-green hover:bg-brand-green/10 rounded-full transition-colors">
                            <Volume2 size={20} />
                        </button>
                    )}
                </div>
                ...
                <div>
                    ...
                    {data.tags && (
                        <div className="flex flex-wrap gap-2 mt-3">
                            {data.tags.map(tag => (
                                <span key={tag} className="text-[11px] bg-slate-100 text-neutral-ink font-bold px-2 py-0.5 rounded-full uppercase tracking-wider">
                                    {tag}
                                </span>
                            ))}
                        </div>
            ...
            <div className="flex gap-6">
                ...
                <div className="flex-1 space-y-3">
                    <div className="grid grid-cols-[80px,1fr] gap-2 items-start">
                        <span className="text-xs font-bold text-neutral-ink uppercase tracking-widest pt-1">Onyomi</span>
                        <span className="text-red-500 font-bold text-lg font-jp">{data.onyomi || "---"}</span>
                    </div>
                    <div className="grid grid-cols-[80px,1fr] gap-2 items-start">
                        <span className="text-xs font-bold text-neutral-ink uppercase tracking-widest pt-1">Kunyomi</span>
                    ...
                    <div className="grid grid-cols-[80px,1fr] gap-2 items-center">
                        <span className="text-xs font-bold text-neutral-ink uppercase tracking-widest">Strokes</span>
                        <span className="text-slate-700 font-bold">{data.strokes || "--"}</span>
                    </div>
                </div>
            </div>
            <div className="mt-6 pt-5 border-t border-slate-50">
                <h4 className="text-xs font-bold text-neutral-ink uppercase tracking-widest mb-2">Meaning</h4>
                <p className="text-xl text-neutral-ink font-bold">{data.meaning}</p>
            </div>
        </div>
    );
};
    ...
    return (
        <div className="bg-white border border-slate-100 rounded-2xl p-6  hover:border-brand-green/30 transition-all">
            <div className="flex items-center gap-3 mb-4">
                ...
                <h3 className="text-xl font-bold text-neutral-ink">{data.head}</h3>
            </div>
            ...
            {data.example && (
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 border-l-4 border-l-brand-green">
                    ...
                    <p className="text-neutral-ink text-sm italic">{data.example.split('(')[1]?.replace(')', '')}</p>
                </div>
            )}
        </div>
    );
            ...
            <div className="flex gap-4">
                ...
                <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                        <p className="font-jp text-xl text-neutral-ink leading-snug group-hover:text-brand-green transition-colors">
                            {data.ja}
                        </p>
                        <Volume2 size={16} className="text-slate-300 cursor-pointer hover:text-brand-dark" />
                    </div>
                    <p className="text-neutral-ink font-medium leading-relaxed">
                        {data.en}
                    </p>
                </div>
            </div>
