            ...
            {exams.map((exam) => (
                ...
                >
                    ...
                    <div className="flex items-start justify-between mb-4">
                        <div className="flex-1 space-y-2">
                            <div className="flex items-center gap-3">
                                ...
                                {exam.isPublic ? (
                                    ...
                                    <span className="flex items-center gap-1.5 px-2.5 py-1 bg-muted text-neutral-ink border border-border/50 rounded-full text-[9px] font-black font-display uppercase tracking-widest ">
                                        <Lock size={10} />
                                        Private
                                    </span>
                                )}
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="p-3 hover:bg-muted rounded-xl transition-all  bg-card border border-border/50 text-neutral-ink hover:text-foreground active:scale-90"
                            >
                                ...
                                <>
                                    ...
                                    <div className="absolute right-0 top-full mt-2 w-48 bg-card rounded-[1.5rem]  border border-border/50 py-2 z-20 animate-in zoom-in-95 duration-200">
                                        <button
                                            ...
                                            className="w-full flex items-center gap-3 px-4 py-3 text-[10px] font-black font-display uppercase tracking-widest text-neutral-ink hover:text-primary hover:bg-muted/30 transition-all border-b border-border/10"
                                        >
                                            <Edit3 size={14} />
                                            Modify Exam
                                        </button>
                    ...
                    {exam.config.description && (
                        <p className="text-sm font-bold text-neutral-ink mb-6 line-clamp-2 leading-relaxed">
                            {exam.config.description}
                        </p>
                    )}
                    ...
                    <div className="flex items-center gap-6 text-[10px] font-black uppercase tracking-widest text-neutral-ink font-display mb-8">
                        {exam.config.tags.timerMode !== 'UNLIMITED' && exam.config.stats.timeLimitMinutes && (
                            <span className="flex items-center gap-2">
                                <Clock size={14} />
                                <span className="text-neutral-ink">{exam.config.stats.timeLimitMinutes}</span> MINS
                            </span>
                        )}
                        {exam.timesAttempted > 0 && (
                            <span className="flex items-center gap-2">
                                <Users size={14} />
                                <span className="text-neutral-ink">{exam.timesAttempted}</span> ATTEMPTS
                            </span>
                        )}
                        ...
                        <div className="absolute inset-0 z-30 bg-background/95  rounded-2xl flex flex-col items-center justify-center p-8 text-center animate-in fade-in duration-300">
                            ...
                            <div className="flex gap-4 w-full max-w-xs">
                                <button
                                    ...
                                    className="flex-1 py-3 bg-card border border-border/50 rounded-xl text-[10px] font-black font-display uppercase tracking-widest text-neutral-ink hover:text-foreground transition-all "
                                >
                                    Cancel
                                </button>
                                <button
