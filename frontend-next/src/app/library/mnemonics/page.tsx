...
function KanjiCard({ kanji, uri }: KanjiCardProps) {
  ...
  const stateStyles: Record<CardState, { bg: string; border: string; label: string; badge: string }> = {
    unknown: {
      bg: "bg-card",
  ...
  const currentStyles = stateStyles[cardState];

  return (
    <div
      className={`group relative flex flex-col items-center justify-center aspect-square text-center rounded-2xl border transition-all duration-300 hover:  ${currentStyles.bg} ${currentStyles.border}`}
    >
      {/* Toggle button - Tactile corner indicator */}
      <button
        onClick={handleToggleState}
        className={`absolute top-2 right-2 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black uppercase tracking-tighter  transition-transform active:scale-90 ${currentStyles.badge}`}
        title={`Status: ${cardState}`}
      >
        {currentStyles.label}
      </button>
      ...
      {/* Kanji link */}
      <Link
        href={`https://www.kanjidamage.com${uri}`}
        target="_blank"
        rel="noopener noreferrer"
        className="flex items-center justify-center w-full h-full p-4"
      >
        <span className="text-4xl md:text-5xl font-black text-foreground font-jp group-hover:text-primary transition-colors duration-300">
          {kanji}
        </span>
      </Link>
    </div>
...
export default function KanjiPage() {
  ...
  const tabs = [
    { label: "N5", level: "N5", data: kanjiDataN5 },
    { label: "N4", level: "N4", data: kanjiDataN4 },
    { label: "N3", level: "N3", data: kanjiDataN3 },
    { label: "N2", level: "N2", data: kanjiDataN2 },
    { label: "N1", level: "N1", data: kanjiDataN1 },
  ];
      ...
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="bg-card rounded-2xl p-8 md:p-12 border border-border  text-center relative overflow-hidden">
          ...
          <h1 className="text-4xl md:text-5xl font-black text-foreground mb-6 font-display tracking-tight">
            JLPT <span className="text-primary">Graded</span> Kanji Cards
          </h1>
        ...
        {/* Tab Interface */}
        <div className="bg-muted/30 p-2 rounded-2xl flex flex-wrap justify-center gap-2 border border-border/50">
          {tabs.map((tab) => {
        ...
        {/* Content Area */}
        {tabs.map((tab) => {
          if (tab.level !== activeTab) return null;
          return (
            <div key={tab.level} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="flex items-center gap-4">
                <div className="h-px flex-1 bg-border"></div>
                <h2 className="text-xl font-black text-foreground font-display uppercase tracking-widest px-4">
                  {tab.level} Mastery
                </h2>
                <div className="h-px flex-1 bg-border"></div>
              </div>
