...
└───────────────┬─────────────┬─────────────┬─────────────┬─────────────┬────────────┘
                │             │             │             │             │
    ┌───────────┴───────────┐ │             │             │             │
    │ /e-api/*              │ │ /f-api/*    │ /h-api/*    │ /s-api/*    │ /d-api/*
    ▼                       ▼ │             ▼             ▼             ▼
┌──────────┐          ┌──────▼─────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
│ Express  │          │   Flask    │  │ Hanachan │  │Study-Plan│  │Dictionary│
│  :8000   │          │   :5100    │  │  :5400   │  │  :5500   │  │  :5200   │
│ Auth/API │          │ Resources  │  │ AI Agent │  │ Goals    │  │ NLP/MeCab│
└────┬─────┘          └──────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘
     │                       │             │             │             │
     │                   ┌───┴────┐        │             │             │
     │                   │ Redis  │←───────┘             │             │
   ...
   ▼             ▼                   ▼             ▼                   ▼
┌───────┐  ┌─────────┐         ┌─────────┐  ┌─────────┐           ┌─────────┐
│MongoDB│  │ Qdrant  │         │  Neo4j  │  │ SQLite  │           │ MeCab   │
│ :27017│  │ :6333   │         │  :7687  │  │ (local) │           │ (Local) │
│ (Docs)│  │(Vectors)│         │ (Graph) │  │ (Chat)  │           │ (Token) │
└───────┘  └─────────┘         └─────────┘  └─────────┘           └─────────┘
```
...
```bash
# Clone and setup
git clone https://github.com/your-username/hanachan.org.git
cd hanachan.org
